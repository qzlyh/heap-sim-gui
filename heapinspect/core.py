...
class HeapInspector(object):
    ...
    def tcache_chunks(self):
        ...
        for index, entry_ptr in enumerate(self.tcache.entries):
            ...
            while entry_ptr:
                ...
                if(self.libc_version in ["2.32","2.33","2.34"]): # libc2.32 - tcache->fd = tcache->next & tcache->addr >> 12
                    chunk.fd = chunk.fd ^ (chunk._addr>>12)
                lst.append(chunk)
                entry_ptr = chunk.fd
    ...
    ''' lyh 2021-11-16 start '''
    @property
    def top(self):
        top_ptr = self.main_arena.top
        mem = self.proc.read(top_ptr, 4*self.size_t)
        chunk = self.MallocChunk._new(mem, top_ptr)
        return chunk
        
    @property
    def last_remainder(self):
        last_remainder_ptr = self.main_arena.last_remainder
        mem = self.proc.read(last_remainder_ptr, 4*self.size_t)
        chunk = self.MallocChunk._new(mem, last_remainder_ptr)
        return chunk
    ''' lyh 2021-11-16 end '''
